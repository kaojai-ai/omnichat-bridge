...
test("LINE OA retains sticker identity, a display URL, and its send correlation ID", () => {
  const adapter = createRegistry({ includeLineOA: true }).get("line_oa");
  const messages = adapter.normalizeMessages({ provider_account_id: "@test", messages: [{ type: "message", timestamp: 1000, source: { chatId: "chat-1", userId: "user-1" }, message: { id: "msg-1", type: "sticker", packageId: "123", stickerId: "10445608" }, sendId: "provider-send-1" }] }, "history_recovery");
  assert.deepEqual(plain(messages[0].sticker), { package_id: "123", sticker_id: "10445608" });
  assert.equal(messages[0].provider_send_id, "provider-send-1");
